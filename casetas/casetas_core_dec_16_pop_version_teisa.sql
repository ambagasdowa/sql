...
insert into @percents
					select 
						...
						,track.unit
						,pop.no_tarjeta_iave
						,track.iave_iave as 'alpha_num_code'
						,track.alpha_location
						,track._monto_archivo as 'alpha_location_1' -- do not have a use yet 
						,track._filename
						,pop.no_viaje
						,track.fecha_cruce
						,pop.fecha_real_viaje as 'f_despachado'
						,pop.fecha_real_fin_viaje as 'fecha_fin_viaje'
						,track._monto_archivo as 'float_data' --monto again and comes from iave
						,track.hora_cruce
						,track.cia
						,track._monto_archivo as 'Monto_archivo' --this come from iave
						,(null) as '_next'
						-- === Period Section ==== ---
						...
						,case 
							when (track.iave_cruce_id is null and (cast(pop.fecha_real_fin_viaje as date) > (select fecha_fin from sistemas.dbo.casetas_iave_periods where period_iave_id = pop.period_iave_id) ) ) -- posible in next iave
								then
									5
							when (track.iave_cruce_id is null and (cast(pop.fecha_real_fin_viaje as date) <= pop.fecha_fin) ) -- with liq
								then
									5
							-- >
							--when (track.iave_cruce_id is null and ( month(track.fecha_cruce) > month(pop.fecha_ini)) OR ( month(track.fecha_cruce) < month(pop.fecha_inicio))) -- iave fecha_a distinct to a period in same (when not conciliatio)
							--	then
							--		5
							--when (track.iave_cruce_id is null and ( year(track.fecha_cruce) > year(pop.fecha_ini)) OR ( year(track.fecha_cruce) < year(pop.fecha_inicio))) -- iave fecha_a distinct to a period in same (when not conciliatio)
							--	then
							--		5
							---- >
							when (track.iave_cruce_id is not null ) -- successfull cross
								then
									1
						...
						,track.date_cruce
						,track.iave_cruce_id
						,track._monto_archivo
						,track.iave_ejes
						,track.iave_iave -- tarjeta_iave
						,track.iave_caseta_id
					...
					from 
							...
							left join 
										(
											...
											from 
													sistemas.dbo.casetas as _shift
											where 
											...
											on 
												track.alpha_num_code = (cast(pop.iave_catalogo as nvarchar(50)) COLLATE SQL_Latin1_General_CP1_CI_AS) 
											and 
												track.consec = pop.consec --this concec nothing has to be with lis
											and
												cast(replace(track.key_num_5,'.','') as nvarchar(50)) COLLATE SQL_Latin1_General_CP1_CI_AS = (cast(replace(pop.desc_caseta,'.','') as nvarchar(50)) COLLATE SQL_Latin1_General_CP1_CI_AS) 
											and 
												(month(track.fecha_cruce) = month(pop.fecha_ini)) 
											and
		...
		--select substring(replace('LIB. OTE. S.L.P..',' ',''),(len(replace('LIB. OTE. S.L.P..',' ',''))),1)

-- analisis
--select key_num_5,alpha_location,* from @percents where casetas_controls_files_id in (select cast(item as int) as 'item' from sistemas.dbo.fnSplit(@ctrfile, '|'))
