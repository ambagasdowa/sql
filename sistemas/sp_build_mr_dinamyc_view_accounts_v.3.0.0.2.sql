...
------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  --call to table and save as table var 
	declare @storeTransaction table(
			...
			Compa��a			nvarchar (255)  collate SQL_Latin1_General_CP1_CI_AS,
			...
			tipoTransacci�n		nvarchar (255)  collate SQL_Latin1_General_CP1_CI_AS,
			Referencia			nvarchar (255)  collate SQL_Latin1_General_CP1_CI_AS,
			FechaTransacci�n	nvarchar (255)  collate SQL_Latin1_General_CP1_CI_AS,
			Descripci�n			nvarchar (255)  collate SQL_Latin1_General_CP1_CI_AS,
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
									...
									select 
										...
										,CASE @period_source
											WHEN substring(@period_source,1,4) + '01' 
												...
												--(select PtdBal00 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '02' 
												THEN presupuesto.PtdBal01 
												--(select PtdBal01 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '03' 
												THEN presupuesto.PtdBal02
												--(select PtdBal02 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '04' 
												THEN presupuesto.PtdBal03
												--(select PtdBal03 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '05' 
												THEN presupuesto.PtdBal04
												--(select PtdBal04 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '06' 
												THEN presupuesto.PtdBal05
												--(select PtdBal05 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '07' 
												THEN presupuesto.PtdBal06 
												--(select PtdBal06 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '08' 
												THEN presupuesto.PtdBal07
												--(select PtdBal07 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '09' 
												THEN presupuesto.PtdBal08
												--(select PtdBal08 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '10' 
												THEN presupuesto.PtdBal09
												--(select PtdBal09 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '11' 
												THEN presupuesto.PtdBal10
												--(select PtdBal10 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											WHEN substring(@period_source,1,4) + '12' 
												THEN presupuesto.PtdBal11
												--(select PtdBal11 from integraapp.dbo.AcctHist as hist where hist.Acct = presupuesto.Acct and hist.Sub = presupuesto.Sub and hist.FiscYr = presupuesto.FiscYr and hist.LedgerID = 'PRESUP' + presupuesto.FiscYr)
											ELSE '0' 
										END AS 'Presupuesto'
	...
	insert into sistemas.dbo.mr_source_reports_temps
							select 
									...
									,(select ISNULL(integraapp.dbo.GLTran.CpnyID,source_table.company)) AS Compa��a
									...
									,integraapp.dbo.GLTran.TranType AS TipoTransacci�n
									,integraapp.dbo.GLTran.RefNbr AS Referencia
									--,dbo.GLTran.ExtRefNbr AS RefExterna
									,integraapp.dbo.GLTran.ExtRefNbr AS FechaTransacci�n
									--,integraapp.dbo.GLTran.TranDate AS FechaTransacci�n
									,integraapp.dbo.GLTran.TranDesc AS Descripci�n
...
-- exec dbo.sp_build_mr_view_accounts '20160','TBKORI','OF','|'

--select * from  sistemas.dbo.mr_source_reports_temps where _period='201607'

--select * from  sistemas.dbo.mr_source_reports where _period='201607'

--select * from  sistemas.dbo.mr_source_reports where _period = '201607' and _company = 'TBKORI'