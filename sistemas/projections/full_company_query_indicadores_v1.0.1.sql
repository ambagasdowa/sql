		...
		from 
			...
			inner join (
								select --top(1)
									...
									--,(select (right(CONVERT(VARCHAR(10), trg.fecha_guia, 105), 7))) as 'fecha_guia'
									--,(select(CAST(trg.fecha_guia as DATE))) as 'fecha_guia'
									,(
										case 
											when min(CAST(trg.fecha_guia as DATE)) <> max(CAST(trg.fecha_guia as DATE))
											--when (select (right(CONVERT(VARCHAR(10), min(trg.fecha_guia), 105), 7))) <> (select (right(CONVERT(VARCHAR(10), max(trg.fecha_guia), 105), 7)))
												then 
																(
																	(select(CAST(trg.fecha_guia as DATE)))
																)
											else
																(
								...
								group by trg.id_area,trg.id_fraccion,trg.no_viaje
										...
										--,(select (right(CONVERT(VARCHAR(10), trg.fecha_guia, 105), 7)))
										--,CAST(trg.fecha_guia as DATE)
...
select subtotal,no_guia,fecha_guia,no_viaje,id_fraccion,fecha_modifico,fecha_confirmacion from trafico_guia where no_viaje = @viaje and status_guia <> 'B' and prestamo <> 'P' and tipo_doc = 2 
...
group by guia.no_viaje , guia.fecha_guia
